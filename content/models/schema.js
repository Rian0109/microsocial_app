...
export const createDatabase = () => {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_comment_username ON comments (username);`
  )
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_likes_post_id ON likesPost (post_id);`
  )
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_likes_comment_id ON likesComment (comment_id);`
  )
