...
export const addComment = async (req, res) => {
  ...
  const q = db.prepare(
    `INSERT INTO comments (post_id, username, body, date) VALUES(?, ?, ?, ?);`
  )
...
export const deleteCommentById = async (req, res) => {
  ...
  if (!comment_id) return res.status(400).json({ error: 'Invalid Id' })
...
export const getCommentsByPostId = async (req, res) => {
  ...
  if (!post_id) return res.status(400).json({ error: "Invalid Post's Id" })
